  ...
  <div>
    ...
    <div class="new_tab">
      <Tabs v-model="orderDataStatus" @on-click="selectChange2">
        ...
        <TabPane
          :label="'待发货(' + (tablists.unshipped || 0) + ')'"
          name="1"
        />
        <TabPane
          :label="'待核销(' + (tablists.write_off || 0) + ')'"
          name="5"
        />
    ...
    >
        ...
        <template #content="{ row }">
          <div class="tdinfo">
            ...
            <Row class="expand-row">
              ...
              <Col span="8">
                <span class="expand-key">供应商名称：</span>
                <span
                  class="expand-value"
                  v-text="row.supplier_name || '--'"
                ></span>
              </Col>
            </Row>
          </div>
            ...
            <Dropdown transfer @on-click="allPages">
              <a href="javascript:void(0)" class="acea-row row-middle">
                <span
                  >全选({{
                    isAll == 1
                      ? page.total - checkUidList.length
                      : checkUidList.length
                  }})</span
                >
                <Icon type="ios-arrow-down"></Icon>
          ...
          <Tooltip :transfer="true" theme="dark" max-width="300" :delay="600">
            <div class="tabBox" v-for="(val, i) in row._info" :key="i">
              <div class="tabBox_img" v-viewer>
                <img v-lazy="val.cart_info.productInfo.image" />
              </div>
            ...
            <div slot="content">
              <div class="mb-10" v-for="(val, i) in row._info" :key="i">
                <div >
                  <span class="font-color-red" v-if="val.cart_info.is_gift"
                    >赠品</span
                  >
                  <span>{{ val.cart_info.productInfo.store_name }}</span>
                </div>
                <div class="flex-y-center">规格：{{val.cart_info.productInfo.attrInfo.suk || ""}}</div>
                <div class="flex-y-center">{{"￥" +val.cart_info.sum_price + " x " + val.cart_info.cart_num}}</div>
              </div>
            </div>
          </Tooltip>
        </template>
      ...
      <vxe-column
        ...
        min-width="150"
      ></vxe-column>
      <vxe-column field="pay_type_name" title="支付方式" min-width="100">
        <template v-slot="{ row }">
          <span>{{ row.pay_type_name }}</span>
        </template>
      </vxe-column>
      <vxe-column field="mark" title="备注" min-width="120">
        <template v-slot="{ row }">
          <span>{{ row.mark || "--" }}</span>
        </template>
      </vxe-column>
      <vxe-column field="remark" title="商家备注" min-width="120">
        <template v-slot="{ row }">
      ...
      <vxe-column field="statusName" title="订单状态" min-width="100">
        <template v-slot="{ row }">
          ...
          <Tag
            color="orange"
            size="medium"
            v-show="[1, 2, 5].includes(row.status)"
      ...
      >
        <template v-slot="{ row }">
          ...
          <a
            ...
            @click="btnClick(row)"
            v-if="
              row.supplier_id !== 0 && row.status_name.status_name == '未发货'
            "
            >提醒发货</a
          >
          <Divider
    ...
    >
      <Form :label-width="100">
        <FormItem label="退款金额：" required>
          <Input type="number" v-model="refundMoney" style="width: 408px;" />
          <!-- <InputNumber
            v-model="refundMoney"
            style="width: 408px;"
          ></InputNumber> -->
        ...
        <FormItem v-if="refundProductNum > 1" label="分单退款：">
          ...
          <Table
            v-show="is_split_order"
            ref="refundTable"
            max-height="500"
            :columns="refundColumns"
            :data="refundProduct"
            @on-selection-change="refundSelectionChange"
          >
            <template slot-scope="{ row }" slot="product">
              ...
              >
                <div>
                  ...
                  <img :src="row.productInfo.attrInfo.image" class="image" />
                </div>
                <div class="title line2">{{ row.productInfo.store_name }}</div>
                </div>
                <template #content>
                  <div class="title">商品名称:{{ row.productInfo.store_name }}</div>
                  <div class="title">商品规格：{{ row.productInfo.attrInfo.suk }}</div>
                </template>
              </Tooltip>
            </template>
            <template slot-scope="{ row }" slot="action">
              <InputNumber
                ...
                :max="row.cart_num - row.refund_num"
                :min="1"
                :precision="0"
                controls-outside
                @on-change="refundNumChange(row)"
              ></InputNumber>
            </template>
          </Table>
        </FormItem>
...
export default {
  ...
  computed: {
    ...
    refundProductNum() {
      return this.refundProduct.reduce(
        (total, { refundNum }) => total + refundNum,
        0
      );
    },
  },
    ...
    refundSelection: {
      handler(value) {
        ...
        this.refundMoney = value.reduce((total, { refundPrice, refundNum }) => {
          return this.$computes.Add(
            total,
            this.$computes.Mul(refundPrice, refundNum)
          );
        }, 0);
      },
      deep: true,
  ...
  methods: {
    ...
    printOreder() {
      if (
        this.checkUidList.length > 10 ||
        (this.isAll == 1 && this.page.total > 10)
      ) {
    ...
    getOnlyRefundData(id, refund_type, rowActive) {
      ...
      console.log(_info);
      let cart_info = Object.keys(_info)
        .map((key) => {
      ...
      cart_info.forEach((value) => {
        value.refundPrice = this.$computes.Div(
          value.refund_price,
          value.cart_num
        );
    ...
    // 发送货
    sendOrder(row, num) {
      if (row.is_stock_up == 1 && row.status == 0)
        return this.$Message.warning("商家寄件，等待平台返回快递单号");
      ...
      this.$refs.send.getBusineseList();
      this.$nextTick((e) => {
