...
class SystemStoreServices extends BaseServices
{
    ...
    public function getStoreList(array $where, array $field = ['*'], string $latitude = '', string $longitude = '', int $product_id = 0)
    {
        ...
        foreach ($storeList as &$item) {
            if (isset($item['distance'])) {
                $item['range'] = $item['distance'] < 1000 ? $item['distance']."m" : bcdiv($item['distance'], '1000', 1)."km";
            } else {
                $item['range'] = "0";
            }
            if ($item['is_show'] == 1) {
    ...
    public function saveStore(int $id, array $data, array $staff_data = [])
    {
        ...
        $this->dao->cacheUpdate($info->toArray(), $id);
        return $id;
    }
    ...
    public function getStoreDisposeCache(int $id, string $felid = '')
    {
        ...
        if ($storeInfo) {
            ...
            $storeInfo['dataVal'] = $storeInfo['valid_time'] ? (is_array($storeInfo['valid_time']) ? $storeInfo['valid_time'] : explode(' - ', $storeInfo['valid_time'])) : [];
            $storeInfo['timeVal'] = $storeInfo['day_time'] ? (is_array($storeInfo['day_time']) ? $storeInfo['day_time'] : explode(' - ', $storeInfo['day_time'])) : [];
            $storeInfo['address2'] = $storeInfo['address'] ? (is_array($storeInfo['address']) ? $storeInfo['address'] : explode(' - ', $storeInfo['address'])) : [];
            return $storeInfo;
        }
        return [];
    }
